...
public class StoryBuddiesBaseActivity extends Activity {
	...
	public void onResume() {
		...
		//--nobt mAudioManager = (AudioManager) getSystemService(AUDIO_SERVICE);
		//--nobt mAudioManager.setSpeakerphoneOn(true);
		
		//--nonfc Intent intent = getIntent();
		//--nonfc if (NfcAdapter.ACTION_NDEF_DISCOVERED.equals(intent.getAction())) {
		//--nonfc	handleNFCIntent(intent);
		//--nonfc }
	...
	private List<StoryBook> getBuiltInStories(){
		...
		if (true) //--nonfc myAnimal != null && myAnimal.equals("hare"))
		{
		...
		if (true) //--nonfc myAnimal != null && myAnimal.equals("bear"))
		{
